...
class Flag(object):
    ...
    layer_size = [[64, 64], [30, 30], [12, 12], [4, 4], [1, 1]]
    kernel_size = [[5, 5], [5, 5], [5, 5], [4, 4]]
    kernel_depth = [1, 10, 10, 10, 10, 1]
...
class Network(object):
    ...
    def build_Y(self, X):
        ...
        for ind_layer in range(n_layers):
            ...
            W = tf.Variable(tf.zeros([kernel_size[0], kernel_size[1], kernel_depth_in, kernel_depth_out]),
                            name='W_{}'.format(ind_layer))
            b = tf.Variable(tf.zeros([1, 1, 1, kernel_depth_out]),
                            name='b_{}'.format(ind_layer))

            params.update({W.name : W, b.name : b})

            if layer_type == 'conv':

                out_pointer = tf.nn.conv2d(in_pointer, W, strides= [1, 1, 1, 1], padding = 'VALID') + b
                in_pointer = out_pointer
            elif layer_type == 'convstride':

                out_pointer = tf.nn.conv2d(in_pointer, W, strides= [1, 2, 2, 1], padding = 'VALID') + b
                in_pointer = out_pointer
...
def main():
    ...
    # cost = tf.reduce_mean(tf.divide(tf.squared_difference(tf.pow(10.0, Y), tf.pow(10.0, Y_), name = 'squar_diff'),
    #                                 tf.square( tf.pow(10.0, Y_)*ln10*errM_)), name= 'cost')

    # todo switch between
    # cost = tf.reduce_mean(tf.divide(tf.squared_difference(1.0, tf.pow(10.0, Y_ - Y)),
    #                                 tf.square(ln10 * errM_)), name='cost')
    # todo remove 11!
    cost = tf.reduce_mean(tf.divide(tf.squared_difference(1.0, tf.pow(10.0, Y_ - Y - 11.0)),
                                                                    tf.square(ln10 * errM_)), name='cost')
    ...
    def info_foo(x_in, y_in, errM):

        [c, y] = sess.run([cost, Y], feed_dict={X: x_in, Y_: y_in, errM_: errM})
    ...
    def training_step(i, update_test_data, update_train_data):
        ...
        if update_train_data:
            ...
            c = info_foo(batch_X, batch_Y_mass, errM)
            c_t.append(c)
        ...
        if update_test_data:
            ...
            c = info_foo(batch_X_te, batch_Y_mass_te, errM_te)

            c_t_test.append(c)

        # the backpropagation training step
        sess.run(train_step, feed_dict={X: batch_X, Y_: batch_Y_mass, errM_: errM})
